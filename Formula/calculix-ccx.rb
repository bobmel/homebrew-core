class CalculixCcx < Formula
  ...
  def install
    ...
    fflags = %w[-Wall -O3]
    fflags << "-fopenmp" if build.with? "libomp"
    cflags = %w[-Wall -O3 -I../../spooles -DARCH="Linux" -DSPOOLES -DARPACK -DMATRIXSTORAGE -DNETWORKOUT]
    cflags << "-DUSE_MT=1" if build.with? "libomp"
    arpacklib = `pkg-config --libs arpack`.chomp
    libs = %w[../../spooles/spooles.a -framework accelerate]
    libs << arpacklib
    libs << "-lomp" if build.with? "libomp"
    args = %W[
      FC=gfortran
      CFLAGS=#{cflags.join(" ")}
      FFLAGS=#{fflags.join(" ")}
